...
// RegionNamesIncludingDropping returns all regions on the multi-region
// enum including regions in the process of being dropped. Since the zone
...
func (desc *immutable) RegionNamesIncludingDropping() (descpb.RegionNames, error) {
	if desc.Kind != descpb.TypeDescriptor_MULTIREGION_ENUM {
		return nil, errors.AssertionFailedf(
			"can not get regions of a non multi-region enum %d", desc.ID,
		)
