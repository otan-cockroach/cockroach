...
func (d *delegator) delegateShowTables(n *tree.ShowTables) (tree.Statement, error) {
	...
	if n.ExplicitSchema {
		...
		schemaClause = fmt.Sprintf("AND ct.schema_name = %s", schema)
	} else {
		...
		schemaClause = "AND ct.schema_name NOT IN ('information_schema', 'pg_catalog', 'crdb_internal', 'pg_extension')"
	}

	const getTablesQuery = `
SELECT ct.schema_name AS schema_name,
