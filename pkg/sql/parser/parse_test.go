...
func TestUnimplementedSyntax(t *testing.T) {
	testData := []struct {
		...
		{`CREATE TEMP TABLE (a int) ON COMMIT DROP`, 46556, ``},
		{`CREATE TEMP TABLE (a int) ON COMMIT DELETE ROWS`, 46556, ``},
		{`CREATE TEMP TABLE IF NOT EXISTS (a int) ON COMMIT DROP`, 46556, ``},
		{`CREATE TEMP TABLE IF NOT EXISTS (a int) ON COMMIT DELETE ROWS`, 46556, ``},
		{`CREATE TEMP TABLE b AS SELECT a FROM a ON COMMIT DROP`, 46556, ``},
		{`CREATE TEMP TABLE b AS SELECT a FROM a ON COMMIT DELETE ROWS`, 46556, ``},
		{`CREATE TEMP TABLE IF NOT EXISTS b AS SELECT a FROM a ON COMMIT DROP`, 46556, ``},
		{`CREATE TEMP TABLE IF NOT EXISTS b AS SELECT a FROM a ON COMMIT DELETE ROWS`, 46556, ``},
