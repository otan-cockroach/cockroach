...
type DistanceCalculator interface {
	...
	// PointInPolygon returns whether the point is inside the given polygon.
	PointInPolygon(point Point, polygon Polygon) bool
...
func onPointToPolygon(c DistanceCalculator, a Point, b Polygon) bool {
	...
	if c.PointInPolygon(a, b) {
		return c.DistanceUpdater().OnIntersects()
	}
	for ringIdx := 0; ringIdx < b.NumLinearRings(); ringIdx++ {
		// Note we don't have to check against the first vertex of the linearRing, as it
		// will always appear in V1 at the end.
		if onPointToEdgeEnds(c, a, b.LinearRing(ringIdx)) {
			return true
		}
	}
	return false
