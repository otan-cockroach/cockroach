...
func (g *s2GeodistLineString) Vertex(i int) geodist.Point {
	return &s2GeodistPoint{(*g.Polyline)[i]}
}
...
func (g *s2GeodistLinearRing) Vertex(i int) geodist.Point {
	return &s2GeodistPoint{g.Loop.Vertex(i)}
}
...
func distanceSpheroidRegions(
	...
	for _, aRegion := range aRegions {
		for _, bRegion := range bRegions {
			...
			earlyExit, err := onRegionToRegion(minDistanceUpdater, aRegion, bRegion)
			if err != nil {
				return 0, err
			}
...
// PointInPolygon implements geodist.DistanceCalculator.
func (c *spheroidDistanceCalculator) PointInPolygon(
	point geodist.Point, polygon geodist.Polygon,
) bool {
	return polygon.(*s2GeodistPolygon).ContainsPoint(point.(*s2GeodistPoint).Point)
}
...
func onRegionToRegion(u *spheroidMinDistanceUpdater, a s2.Region, b s2.Region) (bool, error) {
	aGeodist, err := regionToGeodistShape(a)
	if err != nil {
		return false, err
	}
	bGeodist, err := regionToGeodistShape(b)
	if err != nil {
		return false, err
	}
	return geodist.ShapeDistance(&spheroidDistanceCalculator{updater: u}, aGeodist, bGeodist)
}
