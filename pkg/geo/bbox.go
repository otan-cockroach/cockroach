...
func BoundingBoxFromGeom(g geom.T) (geopb.BoundingBox, error) {
	bbox := geopb.MakeBoundingBox()
	err := boundingBoxFromGeom(g, &bbox)
	return bbox, err
}

func boundingBoxFromGeom(g geom.T, bbox *geopb.BoundingBox) error {
	switch g := g.(type) {
	case *geom.Point:
		bbox.Update(g.X(), g.Y())
	...
	case *geom.GeometryCollection:
		for i := 0; i < g.NumGeoms(); i++ {
			err := boundingBoxFromGeom(g.Geom(i), bbox)
			if err != nil {
				return err
			}
		}
	default:
		return errors.Newf("unknown geom type: %T", g)
	}
	return nil
}
