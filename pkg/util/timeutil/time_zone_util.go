...
const (
	...
	// offsetBoundSecs is the maximum offset a timezone can have.
	offsetBoundSecs = 167*60*60 + 59*60
)

// timezoneOffsetRegex is a regex for timezone offsets which appear in the form
// `GMT±<offset>` or `UTC±<offset>`.
var timezoneOffsetRegex = regexp.MustCompile(`(?i)^(GMT|UTC)([+-])(\d{1,3}(:\d{1,2}){0,2})$`)
