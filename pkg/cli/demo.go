...
const maxNodeInitTime = 10 * time.Second
...
func setupTransientCluster(
	...
	// allServsReadyCh is used to account for all servers successfully calling ReadyFn.
	...
	for i := 0; i < demoCtx.nodes; i++ {
		...
		// servReadyCh is used if latency simulation is requested to notify that a test server has
		// successfully computed its RPC address.
		servReadyCh := make(chan struct{})

		if demoCtx.simulateLatency {
			args.Knobs = base.TestingKnobs{
				Server: &server.TestingKnobs{
					PauseAfterGettingRPCAddress:  latencyMapWaitCh,
					SignalAfterGettingRPCAddress: servReadyCh,
